export const metadata = {
  ...
  date: '2025-04-07',
  summary:
    'Learn how to streamline package management in Nx by breaking down libraries into modular components and automating workflows with custom workspace plugins.',
  title:
    'Breaking Down Libraries in Nx Monorepos with TypeScript Project References',
}

This is the second article in a series on **TypeScript project references in Nx monorepos** and **PNPM workspaces**. You can read the previous article [here](https://moatorres-blog.vercel.app/journal/technology/typescript-project-references-and-pnpm-workspaces-in-nx-monorepos).

## Quick Recap

In the [previous part](https://moatorres-blog.vercel.app/journal/technology/typescript-project-references-and-pnpm-workspaces-in-nx-monorepos) of the series we covered:
...
2. Replace the existing contents of `packages/utils/src/lib/utils.tsx`:
...
<span className="flex justify-center w-full pb-4">
  <sub>Updated `utils.tsx`.</sub>
</span>

3. Update the test suite on `packages/utils/src/lib/utils.spec.tsx`:
...
<span className="flex justify-center w-full pb-4">
  <sub>Updated `utils.spec.tsx`.</sub>
</span>
...
Lastly, remove the content of `apps/sandbox/src/app/global.css` and import `tailwindcss`:
...
![Sandbox App Preview](/images/journal/nx-prefs-002.png)

<span className="flex justify-center w-full pb-4">
  <sub>`nx run-many` output.</sub>
</span>
...
2. Install any required dependencies, like `class-variance-authority`:
...
3. Replace the contents of `packages/button/src/lib/button.tsx`:
...
<span className="flex justify-center w-full pb-4">
  <sub>Updated `button.tsx`.</sub>
</span>

4. Install `@nx-prefs/utils`:
...
5. Remove unused files, such as `button.module.css`, and update the test suite on `packages/button/src/lib/button.spec.tsx`:
...
<span className="flex justify-center w-full pb-4">
  <sub>Updated `button.spec.tsx`.</sub>
</span>
...
If you followed the previous article, Nx will has automatically synced your workspace. If not, when prompted, hit `Enter`. To manually sync your workspace, run `nx sync` from the root of your workspace.
...
We’ll also need to configure Tailwind to recognize and load styles from our package, which can be done using the [`@source`](https://tailwindcss.com/docs/functions-and-directives) directive:
...
Custom plugins can enforce consistent naming, configuration, README files or component templates. We can wrap all those repetitive steps and best practices into a single, streamlined generator.
...
**Note**: We can also write our own custom executors to define how tasks should run for specific tooling or workflows.
...
Generators can help us save time by enforcing consistent patterns and reducing manual setup and configuration. For example:
...
This would generate a new React component named `MyComponent` inside the `my-lib-or-app` project. Generators can also be customized or created from scratch to fit your team's conventions.

## Building a Custom Nx Plugin

Let’s walk through the process of building a custom Nx plugin to scaffold libraries consistently and efficiently. We'll use the [`@nx/plugin`](https://nx.dev/nx-api/plugin) package to help us create and customize the plugin.
...
Next, generate the plugin:

```sh
nx generate @nx/plugin:plugin \
  --name=plugin \
  --linter=eslint \
  --unitTestRunner=jest \
  --useProjectJson=true \
...
This will create a new package named `plugin` at `packages/plugin`, and importable as `@nx-prefs/plugin`.

If you run into a `Project Graph Error`, you can reset the Nx cache and stop the daemon by running:
...
- **schema.d.ts**: By convention, input options for our generator’s options are defined here.

- **schema.json**: Defines the input options for your generator, including validation and descriptions, and powers both CLI usage and visual interaction through [Nx Console](https://marketplace.visualstudio.com/items?itemName=nrwl.angular-console).
...
- Automating config for bundler, linter, test runner, and import path.
- Adding a README template to every new package.
...
First, update `schema.d.ts` to include variables required by our templates:
...
Then replace the contents of `generator.ts` with the following:
...
function normalizeOptions(
  ...
  const projectRoot = options.directory ?? `packages/${options.name}`
...
Now let’s set up our file templates. Copy the structure from an existing library like `button` into our generator’s `files/` directory. Add the `.template` suffix to each file:
...
Each file can now use variables like `<%= name %>`, `<%= importPath %>`, and `<%= offsetFromRoot %>` to dynamically inject values.
...
<sup>Update both `variable`'s name and value, or copy and paste as shown.</sup>
...
<sup>Update `baseConfig`'s import.</sup>
...
<sup>
  Update `cacheDir` inside `defineConfig` and `name`—under
  `build.lib`—properties as shown.
</sup>
...
Replace the contents of `packages/plugins/src/index.ts` with:
...
We can now generate packages using our custom plugin! 🎉
...
Then import and use it inside `apps/sandbox/src/app/page.tsx`:
...
![Sandbox App](/images/journal/nx-prefs-003.png)
