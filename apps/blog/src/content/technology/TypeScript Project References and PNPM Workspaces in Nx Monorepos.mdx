...
This article is a response to an [invitation](https://github.com/nrwl/nx/issues/4508#issuecomment-2761891914) to share my experience using [Nx with TypeScript Project References](https://nx.dev/blog/typescript-project-references), as well as a testament to the kindness and support within the developer community.
...
TypeScript Project References have been available since TypeScript 3.0. According to the official TypeScript [documentation](https://www.typescriptlang.org/docs/handbook/project-references.html):
...
In short, project references let you break a large codebase into smaller, manageable projects that can reference each other while maintaining strict boundaries.
...
Previously, Nx relied heavily TypeScript's `compilerOptions` path mappings. We defined all projects in a central `tsconfig.base.json` or `tsconfig.json` file, which Nx then read—often using functions like [readTsConfigPaths](https://github.com/nrwl/nx/blob/bf8848da951d1570280a51db42ae56a98f918770/packages/nx/src/project-graph/plugins/resolve-plugin.ts#L183) function.
...
There's when Project References come in handy. Project References solve this by allowing each project to explicitly define its dependencies. Instead of centralizing everything in the root `tsconfig.json`, each project can declare its own dependencies (a.k.a. references):
...
We now explicitly define exports and point the `main` and `module` properties to the `./dist/index.js` output. Inter-dependencies are now declared as `"workspace:*"`:
...
While this may sound complicated and challenging to maintain, this is precisely where Nx shines. By offering commands like `nx sync` and `nx watch-deps`, Nx helps alleviate the complexity of managing large-scale monorepos.
...
If you plan to publish packages, ensure your workspace is assigned a valid NPM organization name. For this exercise, I'll use `nx-prefs` as our organization scope.

To create your Nx workspace, run the following command:
...
That's it! We're now ready to use TypeScript Project References. 🚀
...
![VSCode Nx TS Workspace](/assets/journal/nx-prefs-001.png)

I highly recommend exploring the files Nx generated, particularly `pnpm-workspace.yaml`, `tsconfig.base.json`, and `tsconfig.json`. We'll compare these with the path-based workspace on another article.

## Our First Packages

We'll use the official [@nx/react](https://nx.dev/nx-api/react) plugin to create our first package. You can check out Nx's [plugin registry](https://nx.dev/plugin-registry) for a list of all available plugins.
...
<span className="flex justify-center w-full pb-4">
  <sub>Installing `@nx/react`.</sub>
</span>

Nx also has an [official VSCode extension](https://marketplace.visualstudio.com/items?itemName=nrwl.angular-console) that provides an interactive interface for Nx commands—it's definitely worth checking out!
...
<span className="flex justify-center w-full pb-4">
  <sub>Creating `@nx-prefs/utils`.</sub>
</span>
...
Next, we need to add it as a dependency. We can either manually add the dependency to our `package.json` or let PNPM handle it:
...
After adding the package, our `apps/sandbox/package.json` file should look like this:
...
Open your browser on http://localhost:3000 to check if the component renders correctly. Once everything is working, it's time to build our application:
...
Hit `Enter` to sync the workspace and start building our application.
...
If you want to learn more about PNPM workspaces, check out this article by [Juri Strumpflohner](https://github.com/juristr) on [setting up a monorepo with PNPM workspaces](https://nx.dev/blog/setup-a-monorepo-with-pnpm-workspaces-and-speed-it-up-with-nx#consuming-our-sharedui-package-from-the-remix-app).
...
If you're already familiar with Nx, we can take this a step further. In the next part of this series, we'll break down the [shadcn/ui](https://ui.shadcn.com/) into multiple packages using [React](https://react.dev/), [Tailwind](https://tailwindcss.com/), and [Radix UI](https://www.radix-ui.com/), keeping things practical and hands-on.

---

<sub>
  The author is not affiliated to Nx. This article has not been sponsored. The knowledge shared here
  is based on the author's own experience, and should not be treated as source of truth.
</sub>
